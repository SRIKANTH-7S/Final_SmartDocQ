        ...
        if qtype:
            ...
            if qtype_lower == "mcq":
                instr = (
                    ...
                    "Each object must have: question (string), options (array of 4 option strings labeled 'A) ...', 'B) ...', 'C) ...', 'D) ...'), and correct (the correct option label and text, e.g. 'B) Option text'). "
                    "Make sure each option is clearly labeled with A), B), C), D) and the correct answer matches one of these options exactly. "
                )
            elif qtype_lower == "hr":
                instr = (
...
""".replace("{n}", str(num_questions)).replace("{instr}", instr)
            ...
            logging.info(f"generate_questions: Raw MCQ result: {result[:500]}...")
            ...
            try:
                ...
                logging.info(f"generate_questions: Parsed MCQ JSON: {parsed}")
                if isinstance(parsed, list) and len(parsed) >= 1:
                    ...
                    if qtype_lower == "mcq":
                        ...
                        for item in structured:
                            ...
                            # Ensure we have exactly 4 options and they are properly formatted
                            if len(q["options"]) == 4:
                                # Clean up options to ensure they have proper labels
                                cleaned_options = []
                                for i, opt in enumerate(q["options"]):
                                    opt_str = str(opt).strip()
                                    # If option doesn't start with letter, add it
                                    if not re.match(r'^[A-D]\)', opt_str):
                                ...
                                if not re.match(r'^[A-D]\)', correct_ans):
                                    # Try to match with one of the options
                                    for opt in cleaned_options:
                                        if correct_ans.lower() in opt.lower() or opt.lower().replace(')', '').replace(' ', '').endswith(correct_ans.lower()):
                                            correct_ans = opt
                                            break
                        ...
                        if len(validated) >= num_questions:
                            ...
                            logging.info(f"generate_questions: Final validated MCQ questions: {self.structured_questions}")
