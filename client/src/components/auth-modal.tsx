...
export default function AuthModal({ type, onClose, onSwitch, onAuthSuccess }: AuthModalProps) {
  ...
  const onLoginSubmit = async (data: z.infer<typeof loginSchema>) => {
    setIsLoading(true);
    try {
      const res = await fetch(`${(import.meta as any).env?.VITE_API_BASE_URL || "http://localhost:10000/api"}/login`, {
        method: "POST",
  ...
  const onSignupSubmit = async (data: z.infer<typeof signupSchema>) => {
    setIsLoading(true);
    try {
      const res = await fetch(`${(import.meta as any).env?.VITE_API_BASE_URL || "http://localhost:10000/api"}/signup`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
